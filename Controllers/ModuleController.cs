using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Authorization;
using TreinamentosCorp.API.Domain.Services;
using TreinamentosCorp.API.DTOs.Requests;
using TreinamentosCorp.API.DTOs.Responses;

[ApiController]
[Route("api/[controller]")]
public class ModuleController : ControllerBase
{
    private readonly IModuleService _moduloService;

    public ModuleController(IModuleService moduloService)
    {
        _moduloService = moduloService;
    }

    // Upload de vídeo/PDF + criação do módulo
    [HttpPost("upload")]
    [Authorize(Roles = "Administrador,Gestor")]
    [Consumes("multipart/form-data")]
    public async Task<ActionResult<ModuleDTO>> UploadModulo([FromForm] UploadMaterialDto dto)
    {
        if (dto.Arquivo == null || dto.Arquivo.Length == 0)
            return BadRequest("Arquivo inválido.");

        var pasta = Path.Combine("wwwroot", "uploads");
        Directory.CreateDirectory(pasta);

        var caminhoArquivo = Path.Combine(pasta, dto.Arquivo.FileName);

        using (var stream = System.IO.File.Create(caminhoArquivo))
        {
            await dto.Arquivo.CopyToAsync(stream);
        }

        var caminhoPublico = $"/uploads/{dto.Arquivo.FileName}";

        // aqui você adapta para chamar o serviço com IdCurso + caminho do arquivo
        var modulo = await _moduloService.CriarAsync(
        new CreateModuleDTO
        {
            IdCurso = dto.IdCurso,
            // preencha aqui os outros campos obrigatórios (Título, etc.), se existirem
        },
        caminhoPublico
    );

        return Ok(modulo);
    }
    // Listar módulos de um curso
    [HttpGet("curso/{cursoId}")]
    [Authorize]
    public async Task<ActionResult<IEnumerable<ModuleDTO>>> ListarPorCurso(int cursoId)
    {
        var modulos = await _moduloService.ListarPorCursoAsync(cursoId);
        return Ok(modulos);
    }

    [HttpGet("{id}")]
    [Authorize]
    public async Task<ActionResult<ModuleDTO>> GetById(int id)
    {
        var modulo = await _moduloService.GetByIdAsync(id);
        if (modulo == null) return NotFound();

        return Ok(modulo);
    }
}
